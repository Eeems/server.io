	...
	db = module.exports = {
		...
		createIndex: function(table,name,column,callback){
			...
			db.hasIndex(table,name,function(e){
				if(!e){
					db.query("CREATE INDEX "+pool.escapeId(name)+" ON "+pool.escapeId(table)+" ("+pool.escapeId(column)+")",function(){
		...
		update: function(table,id,columns,callback){
			callback = callback===undefined?function(){}:callback;
			db.query("UPDATE "+pool.escapeId(table)+" SET ? WHERE id = ?",[columns,id],function(e,r,f){
